...
if ($Verbose) {
  Write-Host "Copying from $TemplateRoot -> $ExtensionRoot" -ForegroundColor DarkGray
}
...
foreach($file in $TemplateFiles) {
  ...
  if ($Verbose) {
    Write-Host "  copying template file $file..." -ForegroundColor DarkGray
  }
...
Write-Host "Don't forget to add your new extension to the solution."

$pathToSolution = Join-Path $gitRoot "Extensions.sln"
Write-Host @"
